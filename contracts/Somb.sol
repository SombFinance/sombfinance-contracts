...
contract Somb is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 SOMB-WFTM LP -> SOMB pool
    uint256 public constant INITIAL_SOMB_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the SOMB ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("SOMB", "SOMB") {
        // Mints 1 SOMB to contract creator for initial pool setup
    ...
    function _getSombPrice() internal view returns (uint256 _sombPrice) {
        try IOracle(sombOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Somb: failed to fetch SOMB price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _sombPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_sombPrice >= taxTiersTwaps[tierId]) {
    ...
    function setSombOracle(address _sombOracle) public onlyOperatorOrTaxOffice {
        require(_sombOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints SOMB to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of SOMB to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentSombPrice = _getSombPrice();
            currentTaxRate = _updateTaxRate(currentSombPrice);
            if (currentSombPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_sombPool != address(0), "!_sombPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_sombPool, INITIAL_SOMB_POOL_DISTRIBUTION);
